...
public:
  ...
  {
    fixedNumber = false;
    const int numThreads = Get_NumThreads();
    if (numThreads >= 0)
    {
      fixedNumber = true;
      if (numThreads < 1) return 1;
      const UInt32 kNumBZip2ThreadsMax = 64;
      if (static_cast<UInt32>(numThreads) > kNumBZip2ThreadsMax) return kNumBZip2ThreadsMax;
      return static_cast<UInt32>(numThreads);
    }
    return 1;
  }
